...
const SELECT_FIELDS: Record<string, { label: string; value: string }[]> = {
  mail_provider: [
    ...
    { label: 'Official Outlook Web（真实别名）', value: 'outlook_official_web' },
    { label: 'Outlook API（外部邮箱池）', value: 'outlookapi' },
  ],
  default_executor: [
...
const TAB_ITEMS = [
  ...
  },
  {
    ...
    sections: [
      ...
      {
        title: 'Official Outlook Web',
        desc: '直接登录 outlook.live.com 读信，并尝试在微软账号页创建真实 alias',
        fields: [
          { key: 'outlook_official_pool_secret', label: 'Pool Secret 路径', placeholder: '/home/.../pool.json' },
          { key: 'outlook_official_login_slug', label: '登录 slug（可选）', placeholder: 'base@outlook.com----password', secret: true },
          { key: 'outlook_official_base_email', label: '基础 Outlook 邮箱', placeholder: 'base@outlook.com' },
          { key: 'outlook_official_alias_mode', label: 'Alias 模式', placeholder: 'official / base / fixed' },
          { key: 'outlook_official_alias_prefix', label: 'Alias 前缀', placeholder: 'aar' },
          { key: 'outlook_official_target_email', label: '固定目标邮箱（可选）', placeholder: 'customalias@outlook.com' },
          { key: 'outlook_official_poll_interval', label: '轮询间隔（秒）', placeholder: '5' },
          { key: 'outlook_official_timeout', label: '超时（秒）', placeholder: '60' },
        ],
      },
      {
        title: 'Outlook API',
        desc: '借鉴成功样本的外部 Outlook 邮箱池接口，只补邮箱来源，不替换现有 OAuth 主线',
        fields: [
          { key: 'outlook_email_base_url', label: 'API Base URL', placeholder: 'http://127.0.0.1:5000' },
          { key: 'outlook_email_auth_mode', label: '认证模式', placeholder: 'auto / external / internal' },
          { key: 'outlook_email_api_key', label: 'External API Key（可选）', secret: true },
          { key: 'outlook_email_login_password', label: 'Internal Login Password（可选）', secret: true },
          { key: 'outlook_email_group_id', label: 'Group ID（可选）', placeholder: '1' },
          { key: 'outlook_email_address_mode', label: '地址策略', placeholder: 'aliases-first / primary-first / aliases-only / primary-only' },
          { key: 'outlook_email_address_pool', label: '固定地址池（可选）', placeholder: 'a@outlook.com,b@outlook.com' },
          { key: 'outlook_email_folder', label: 'Folder（可选）', placeholder: 'all' },
          { key: 'outlook_email_fetch_top', label: '每次抓取条数', placeholder: '10' },
          { key: 'outlook_email_disable_used_accounts', label: '用后禁用', placeholder: 'true / false' },
          { key: 'outlook_email_disable_used_status', label: '禁用状态', placeholder: 'inactive' },
          { key: 'outlook_email_used_addresses_path', label: '已用地址 ledger 路径', placeholder: '/home/.../outlook-email-used-addresses.json' },
          { key: 'outlook_email_poll_interval', label: '轮询间隔（秒）', placeholder: '5' },
          { key: 'outlook_email_timeout', label: '超时（秒）', placeholder: '60' },
          { key: 'outlook_email_proxy', label: '代理（可选）', placeholder: 'http://127.0.0.1:7890' },
        ],
      },
    ],
  },
